...
const Skills = () => {
    ...
    return (
        <div id="skills" className={`${isDarkMode ? "bg-zinc-900 text-white" : "bg-white text-zinc-900"} h-auto flex flex-col items-center justify-center   md:p-10 p-7`}>
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-3 mt-6">
                {skills.map((skill, index) => (
                    <div
                        key={index}
                        className={`flex flex-row flex-wrap justify-start items-center gap-3  ${isDarkMode ? "bg-zinc-800   " : "  bg-zinc-200 "}   p-3 rounded-lg  transition-transform transform hover:scale-[101%]`}
                    >
